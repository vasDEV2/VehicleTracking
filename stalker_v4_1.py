...
while True:
    ...
    if tracker_locked:
            ...
            if success:
                ...
                distancelog.append(distanceboi)
                anglelog.append(teta)
                vlat.append(vehicle.location.global_relative_frame.lat)
                vlon.append(vehicle.location.global_relative_frame.lon)

                # if curr_time - timerboi >= 3:
                if target_x >= 270 and target_x <= 370:
                    lat,lon = geotag.geotag(distanceboi,pan,vehicle.location.global_relative_frame.lat,vehicle.location.global_relative_frame.lon)
                    ...
                    # timerboi = time.time()
                np.savez('whatisthisdatabrooo', distancelog,anglelog,vlat,vlon)
