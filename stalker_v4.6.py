...
def calc_spd(lt,ln):
    ...
    ds = []
    for i in range(l):
        dur = distance_calculation(lt,ln,lats,lons)
        ds.append(dur)
    xl = min(ds)
    index = ds.index(xl)
    theta = theta_calculator(lats[index+1],lons[index+1],vehicle)
    return(theta)
...
while True:
    ...
    if detected:
        ...
        if wait_detect == False:
            tetew = calc_spd(vehicle.location.global_relative_frame.lat,vehicle.location.global_relative_frame.lon)
            yy = sppeeed*math.cos(math.radians(tetew))
            xx = sppeeed*math.sin(math.radians(tetew))
            send_global_velocity(yy,xx,0)
    else:
        print("no detection")
