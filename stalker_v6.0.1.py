...
while True:
    ...
    if tracker_locked:
            ...
            if success:
                ...
                distancesahihoplz = vehicle.location.global_relative_frame.alt*math.tan(math.radians(udpitch))
                ...
                lat,lon = geotag.geotag(distancesahihoplz,math.radians(p+vehicle.heading),vehicle.location.global_relative_frame.lat,vehicle.location.global_relative_frame.lon)
                duri = distance_calculation(lat,lon,plat,plon)
                dissytaks.append(distanceboi)
                vecloclat.append(vehicle.location.global_relative_frame.lat)
                vecloclon.append(vehicle.location.global_relative_frame.lon)
                np.savez("pitchdistancetune",dissytaks,vecloclat,vecloclon)
                ...
                # yvel = 1*math.cos(math.radians(p+vehicle.heading))
                # xvel = 1*math.sin(math.radians(p+vehicle.heading))
                # if distanceboi >= 5:
