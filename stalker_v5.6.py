...
while True:
    ...
    if tracker_locked:
            ...
            if success:
                ...
                lat1,lon1,distanceboi,teta = geotag.geo_tag_karo(314.70596524,251.9527701,13,pitch,p,vehicle.location.global_relative_frame.lat,vehicle.location.global_relative_frame.lon,vehicle.heading)
                print("Lat: ",lat1,"Lon: ",lon1)
                disx = distanceboi
                disy = distanceboi*math.tan(math.radians(p))
                di = math.sqrt(disx**2 + disy**2)
                print("DDDDDDDDDDD: ",di)
